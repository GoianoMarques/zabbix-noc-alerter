...
function renderList(problems) {
  ...
  // detectar se multi-instance (para mostrar badge)
  ...
  el.innerHTML = problems.map(p => {
    ...
    return `<div class="row ${cls}${p.acknowledged ? ' is-ack' : ''}" role="button" tabindex="0" data-ev="${p.eventid}" data-tid="${p.objectid || ''}" data-hostid="${p.hostid || ''}" data-inst="${p.instId || ''}" title="${t('open_problem', lang)}">
